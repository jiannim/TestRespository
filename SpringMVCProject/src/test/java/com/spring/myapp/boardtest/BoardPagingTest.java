...
public class BoardPagingTest {
	...
	/*
	 1. 페이지 화면 모습
	  ...
	  3. 사용자가 현재 보고 있는 페이지 기중 끝페이지 계산
	  - 만약 현재 보고있는 페이지가 3번이고 한번에 보여줄 페이지가 10페이지라면
	  	-> 끝 페이지 번호는 10번이다.
	  	- 현재 페이지가 37페이지고 한번에 보여줄 페이지가 20개라면
	  	 -> 끝 페이지 번호는 40번이다.
	  	 
	  	 //Math.ceil() : 올림 메서드
	  	 공식 : Math.ceil(현재페이지 번호 / 한페이지당 보여줄 페이지 수) * 한페이지당 보여줄 페이지 수
	  	 
	  	 
	  4. 시작 페이지 번호를 계산하기
	  - 현재 보고있는 페이지가 15번이고 한번에 보여줄 페이지가 10개라면?
	  -> 시작페이지번호  11번, 끝페이지번호 : 20번
	  - 현재 보고있는 페이지가 73번이고 한버에 보여줄 페이지가 30개라면?
	  -> 시작페이지번호 61번, 끝페이지번호 90번
	  
	  - 공식 : (끝 페이지번호 - 한번에 보여줄 페이지 수) +1
	  
	  5. 시작 페이지 번호를 구해낸 후 끝 페이지값 보정
	  - 만약 총 게시불이 138개이고 한 화면당 10페이지를 보여준다면
	  현재 11페이지를 보고있다면 실제 끝 페이지는 20번이 아니라 14번이 나와야 합니다.
	  - 하지만 위 3번 공식을 적용하면 끝 페이지는 20으로 계산되므로 20이라는 숫자는 시작페이지를 구하는 로직에서만 활용하고
	   다시 14로 재보정해야 한다.
